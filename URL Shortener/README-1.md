...
19. [Contributing Guide](#-contributing-guide)
20. [Author](#-author)
21. [Contact](#-contact)
22. [GitHub Stats](#-github-stats)
23. [Star This Repository](#-star-this-repository)
...
flowchart TD
    ...
    G --> F
    F --> E
    E --> H[📄 EJS Template Rendered / Redirect]
    H --> A
```

**In plain English:**

1. A client (browser) sends an HTTP request to the server — either submitting a URL or visiting a short link.
2. **Express Server** receives the raw request.
3. **Middleware** processes the request (e.g. parsing form data).
4. **Express Router** looks at the URL and HTTP method to decide which route handles it.
5. The matched **Route Handler** runs the logic (create URL, redirect, or fetch analytics).
6. **Mongoose** translates that logic into a database read/write operation.
7. **MongoDB Atlas** stores or retrieves the actual document.
8. The result travels back up the chain — either as a **redirect** or a rendered **EJS page**.
...
sequenceDiagram
    ...
    Note over User,Atlas: Shortening a URL
    User->>Express: POST /url (long URL)
    Express->>Router: Route matched
    Router->>Mongoose: Create new URL document (shortId, redirectURL)
    Mongoose->>Atlas: Save document
    Atlas-->>Mongoose: Document saved
    Mongoose-->>Router: shortId returned
    Router-->>User: Shortened URL displayed

    Note over User,Atlas: Visiting a shortened URL
    User->>Express: GET /url/:id
    Express->>Router: Route matched
    Router->>Mongoose: Find document by shortId
    Mongoose->>Atlas: Query executed
    Atlas-->>Mongoose: Document returned
    Mongoose->>Atlas: Push timestamp into visitHistory
    Router-->>User: Redirect to redirectURL
```

This diagram shows two separate journeys: creating a shortened URL, and visiting one — including exactly where the visit gets logged before the redirect happens.
